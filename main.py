...
async def get_statistika(message: Message):
    ...
    if user_id in scores:
        await message.answer(f"📊 <b>Sizning statistikangiz:</b>\n\n👤 Ism: {scores[user_id]['name']}\n🏅 Ball: {scores[user_id]['score']}", parse_mode="HTML")
    else:
...
async def send_question(message, user_id):
    ...
    if test_index >= len(user_tests[user_id]["questions"]):
        await bot.send_message(message.chat.id, f"✅ Test tugadi! Siz {user_tests[user_id]['correct_answers']} ta to‘g‘ri javob berdingiz.", reply_markup=menu_buttons)

        if user_id not in scores:
            scores[user_id] = {"name": message.from_user.full_name, "score": 0}
        scores[user_id]["score"] += user_tests[user_id]["correct_answers"]
